...
| `unparsed-task2` | Грамматика собирается, но не разбирает свои примеры (нет `//`-комментариев и `else`); бот обязан сообщить об этом по выводу ANTLR, а не писать «покрывает все конструкции» |

`eval-prompts.sh` запускает `check.sh` лабы на фикстуре напрямую (без
контейнера) и подставляет его вывод в промпт — так же, как ревьюер. Для
`unparsed-task2` это означает реальную сборку грамматики: нужны `java` и
`antlr-*-complete.jar` (`brew install antlr` на macOS).
