...
концепциям каталога (свойства проектируемого языка из вариантов заданий и
обязательные требования к языку). Маркеры `--8<--` размечают секции для
сайта и не влияют на выполнение.
"""
...
# --8<-- [start:variants-7]
# 7. Перегрузки подпрограмм нет: второе `def` с тем же имеем заменяет
...
def main() -> None:
    ...
    # есть только внутри числовой башни (int → float в арифметике) и в
    # логическом контексте.
    count = int("42")
    ratio = count / 5          # int / int → float неявно
    if count:                  # int → bool неявно
        total += count
    # --8<-- [end:variants-2]
    ...
    # --8<-- [start:variants-4]
    # 4. Область видимости ограничивают только подпрограммы (и классы):
    # переменная, созданная внутри if/for/while, видна после блока.
    if total > 0:
    ...
    # либо бесконечный while с break в конце тела.
    n = 0
    while not n >= 3:
        n += 1
    # --8<-- [end:req-7-2-until]
