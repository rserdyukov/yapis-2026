...
// --8<-- [start:variants-8]
// 8. Параметры получают значения, включая значения ссылок. Copy-значения
// копируются, передача принадлежащего вызывающему не-Copy значения перемещает
// владение. &T — разделяемое заимствование (внутренняя изменяемость возможна),
// &mut T — исключительное заимствование на время активного доступа.
...
fn main() {
    ...
    // --8<-- [start:variants-2]
    // 2. Числовое приведение i32 → f64 явно: `as`; есть также From/Into/parse.
    // Ограниченные неявные coercions существуют, например &String → &str.
    let ratio = count as f64 / 5.0;
    let parsed: i32 = "7".parse().unwrap();
    let text = String::from("coercion");
    let view: &str = &text; // deref coercion
    println!("{view}");
    ...
    // 7.2. Отдельного until нет. Предусловие имитирует while !cond;
    // постусловие repeat-until — loop с проверкой после тела (хотя бы раз).
    let mut n = 0;
    while !(n >= 3) {
        n += 1;
    }
